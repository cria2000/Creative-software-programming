...
def exp(rv) :
    theta = np.sqrt(np.dot(rv[0],rv[0]) + np.dot(rv[1],rv[1]) + np.dot(rv[2], rv[2]))
    theta = l2norm(theta)
    v = normalized(rv)
    return np.array([[np.cos(theta)+((v[0]*v[0])*(1-np.cos(theta))), ((v[0]*v[1])*(1-np.cos(theta)))-(v[2]*np.sin(theta)), (v[0]*v[2]*(1-np.cos(theta))) + v[1]*np.sin(theta)],
